      ...
      <div class="max-w-3xl mx-auto relative z-10">
        ...
        <div class="flex flex-wrap items-center gap-4 text-sm text-zinc-500">
          <div class="flex items-center gap-2">
            <span class="font-medium text-zinc-900">{{ article.author }}</span>
          </div>
          <span>•</span>
      ...
      <div class="prose prose-zinc prose-lg">
        <template v-for="(block, index) in article.content" :key="index">
          ...
          <p v-if="block.type === 'paragraph'" 
             class="text-zinc-600 leading-relaxed">
            {{ block.content }}
          </p>

          <!-- Titre -->
          <h2 v-else-if="block.type === 'heading'" 
              class="text-2xl font-bold text-zinc-900 mt-12 mb-6 scroll-mt-24">
            {{ block.content }}
          </h2>
          ...
          <div v-else-if="block.type === 'list'" class="my-8">
            ...
            <ul class="space-y-3">
              <li v-for="item in block.items" :key="item" 
                  class="flex items-start gap-3 text-zinc-600">
          ...
          <blockquote v-else-if="block.type === 'quote'" 
                      class="my-8 pl-6 py-6 border-l-4"
          ...
          <div v-else-if="block.type === 'code'" 
               class="my-8 rounded-lg overflow-hidden bg-zinc-900 group relative">
          ...
          <figure v-else-if="block.type === 'image'" class="my-8">
            <div class="relative group">
              ...
              <div class="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-lg"></div>
            </div>
